...
(() => {
  ...
  const runProductFlow = async () => {
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      if (clickByText(CLICK_TEXT.viewCart)) {
        return;
      }
      await sleep(250);
    }
  ...
  const runCheckoutFlow = async () => {
    for (let attempt = 0; attempt < 40; attempt += 1) {
      ...
      if (clickByText(CLICK_TEXT.placeOrder)) {
        return;
      }
      await sleep(500);
    }
